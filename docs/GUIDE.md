...
| `sorted(xs)` / `sorted(xs, key=f)` / `sorted(..., reverse=…)` | without `key=`: `list[int\|float\|bool\|str\|orderable tuple\|orderable list]`; with monomorphic `key=`: any `list[T]`; `key=` may be free/nested/lambda or bare `len`/`abs`/`int`/`float`/`bool`/`str`; `reverse=` is truthy (bool/int/str/…) | new sorted list; stable reverse-sort-reverse; keyed path materializes never-freed keys list |
