...
| `min(a, b[, c…])` / `max(…)` | int, float, bool (2+ args) | common numeric type via `bool` → `int` → `float` (ties keep the first arg); optional monomorphic `key=` over homogeneous positionals |
