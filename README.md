...
## The language (v0.34.0)

Versioning is **MAJOR.MINOR.PATCH**. PyRs stays on **0.y.z** (next
milestone after this one is **0.35.0**, not 1.0) until it is ready for
...
Known limits (v0.34.0): `int` is arbitrary precision (tagged small ±2⁶² /
...
iterable form also accept homogeneous `str` and orderable tuples
(lexicographic); multi-arg with
...
`key=` is for `list[int|float|bool|str|orderable tuple]` (empty without `default=` →
...
kwarg); list lexicographic ordering (`[1,2] < [1,3]`) is residual,
...
Release tags: `git tag v0.34.0 && git push origin v0.34.0`.