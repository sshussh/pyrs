...
## The language (v0.78.0)

Versioning is **MAJOR.MINOR.PATCH**. PyRs stays on **0.y.z** (next
milestone after this one is **0.79.0**, not 1.0) until it is ready for
...
- **Classes (v0.21):** `class C:` / `class D(C):` with instance methods
  ...
  **v0.78:** class `<` / `<=` / `>` / `>=` — `__lt__` / `__le__` / `__gt__` /
  ...
  no identity fallback; reflected ops and `sorted`/`min`/`max` of class
  instances remain residual.
...
Known limits (v0.78.0): `int` is arbitrary precision (tagged small ±2⁶² /
...
Release tags: `git tag v0.78.0 && git push origin v0.78.0`.
