...
## The language (v0.55.0)

Versioning is **MAJOR.MINOR.PATCH**. PyRs stays on **0.y.z** (next
milestone after this one is **0.56.0**, not 1.0) until it is ready for
...
- **Expressions:** full arithmetic including `**`, comparisons with
  ...
  `int()`/`float()`/`bool()`/`str()`, `len()`, `abs()`, `round()`, `hex()`/`bin()`/`oct()`, `divmod()`, `min()`/`max()`
...
Known limits (v0.55.0): `int` is arbitrary precision (tagged small ±2⁶² /
...
Release tags: `git tag v0.55.0 && git push origin v0.55.0`.
